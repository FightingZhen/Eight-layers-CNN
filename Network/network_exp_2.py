...
with tf.name_scope('Conv_1'):
    with tf.name_scope('W_conv1'):
        w_conv1=weight_variable([2,2,1,8])
...
with tf.name_scope('Conv_4'):
    with tf.name_scope('W_conv4'):
        w_conv4=weight_variable([2,2,32,64])
...
with tf.name_scope('Conv_5'):
    with tf.name_scope('W_conv5'):
        w_conv5=weight_variable([2,2,64,128])
...
with tf.name_scope('Conv_6'):
    with tf.name_scope('W_conv6'):
        w_conv6=weight_variable([2,2,128,256])
...
with tf.name_scope('Conv_7'):
    with tf.name_scope('W_conv7'):
        w_conv7=weight_variable([2,2,256,512])
    with tf.name_scope('B_conv7'):
        b_conv7=bias_variable([512])
    with tf.name_scope('H_conv7'):
        h_conv7=tf.nn.sigmoid(conv2d(h_pool6,w_conv7)+b_conv7)

with tf.name_scope('Conv_8'):
    with tf.name_scope('W_conv8'):
        w_conv8=weight_variable([2,2,512,1024])
    with tf.name_scope('B_conv8'):
        b_conv8=bias_variable([1024])
    with tf.name_scope('H_conv8'):
        h_conv8=tf.nn.sigmoid(conv2d(h_conv7,w_conv8)+b_conv8)
    with tf.name_scope('H_pool8'):
        h_pool8=max_pool_2x2(h_conv8)

with tf.name_scope('Full_Connected_Layer_1'):
    with tf.name_scope('W_fc1'):
        w_fc1=weight_variable([2*2*1024,1024])
    with tf.name_scope('B_fc1'):
        b_fc1=bias_variable([1024])
    with tf.name_scope('H_pool_flat'):
        h_pool_flat=tf.reshape(h_pool8,[-1,2*2*1024])
